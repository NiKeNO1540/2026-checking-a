...
clear
log_and_echo "╔══════════════════════════════════════════════════════════════╗"
log_and_echo "║         ПРОВЕРКА КОНФИГУРАЦИИ МАРШРУТИЗАТОРА BR-RTR          ║"
...
log_and_echo "│           - 172.16.2.10/28 (WAN-интерфейс к ISP)             │"
log_and_echo "│           - 192.168.3.1/28 (LAN-интерфейс к BR-SRV)          │"
log_and_echo "└──────────────────────────────────────────────────────────────┘"

execute_check "IP-адрес WAN 172.16.2.10/28 (к ISP)" "ip a | grep 172.16.2.10/28"
execute_check "IP-адрес LAN 192.168.3.1/28 (к BR-SRV)" "ip a | grep 192.168.3.1/28"

# ==================== КРИТЕРИЙ 2: HOSTNAME И ВРЕМЕННАЯ ЗОНА ====================
log_and_echo "┌──────────────────────────────────────────────────────────────┐"
log_and_echo "│ КРИТЕРИЙ 2: Проверка имени хоста и временной зоны            │"
log_and_echo "│ Описание: Hostname должен содержать 'br-rtr'                 │"
...
execute_check "Имя хоста содержит 'br-rtr'" "hostnamectl | grep -i br-rtr"
...
if [ -f "$FRR_CONF" ]; then
    ...
    check_config_exists "$FRR_CONF" "network 192.168.3.0/28 area 0" "Сеть 192.168.2.0/28 в area 0 (BR-SRV)"
...
log_and_echo "┌──────────────────────────────────────────────────────────────┐"
log_and_echo "│ КРИТЕРИЙ 8: Проверка GRE-туннеля между BR и HQ               │"
log_and_echo "│ Описание: Проверяем связность с HQ-RTR через туннель         │"
log_and_echo "│           IP-адрес удалённой стороны: 192.168.5.1            │"
log_and_echo "└──────────────────────────────────────────────────────────────┘"
...
execute_check "Ping до HQ-RTR через туннель (192.168.5.1)" "ping -c 4 192.168.5.1"
...
echo "└──────────────────────────────────────────────────────────────┘"
echo ""
echo "  Критерий 1 (IP-адреса):     $(ip a | grep -q '172.16.2.10/28' && ip a | grep -q '192.168.3.1/28' && echo '✓ OK' || echo '✗ FAIL')"
echo "  Критерий 2 (Hostname/Time): $(hostnamectl | grep -qi 'br-rtr' && timedatectl | grep -q 'Asia/Yekaterinburg' && echo '✓ OK' || echo '✗ FAIL')"
...
echo "  Критерий 8 (GRE-туннель):   $(ping -c 1 192.168.5.1 &>/dev/null && echo '✓ OK' || echo '✗ FAIL')"
echo ""
